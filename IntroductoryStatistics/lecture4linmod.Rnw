...
<<echo=F>>=
#write.table(noisedeg,file="noisedeg.txt")
@
...
One instructive exercise is to 
compare these coefficients with the means we have for noise. The mean of no.noise is 500, and the mean of noise is 500+138=638:
...
Next, we have the residuals, whose distribution can be compared to a normal distribution (it's an assumption of the linear model that the residuals be normal; \cite{SenSrivastava} has more details on linear models):
...
In case you are interested in more details, the Sen and Srivastava textbook will give you a fuller introduction.
...
bdata <- read.table("beauty.txt",header=T)
head(bdata)
...
<<fig=TRUE,echo=FALSE,print=FALSE,width=6>>=
