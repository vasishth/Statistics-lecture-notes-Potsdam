...
Suppose $y$ is a vector of continuous responses; assume for now that it is coming from a normal distribution. In other words, the random variable $Y$ has as pdf:

$Y \sim Normal(\mu,\sigma)$

A crucial assumption here is that the vector of data $y$ is \textbf{independent and identically distributed}. This implies that if we have $n$ data points, $y_1,y_2,\dots,y_n$, each one of these data points has the same distribution, and is indepenent from each other.

One can equivalently write the above model as a so-called linear model:

$y = \mu + \varepsilon \hbox{ where } \varepsilon \sim Normal(0,\sigma)$

The goal is to estimate two parameters, $\mu,\sigma$. You already know how to estimate these parameters given $y$.

But now consider the situation where we have not one vector of $y$ responses, but  two vectors. The first vector, of length $n$, is:

$y_a \sim Normal(\mu_a,\sigma)$

and the second vector, also of length $n$, is:

$y_b \sim Normal(\mu_b,\sigma)$

Thus, the vector $y_a$ contains $n$ data points: 
$y_{a1}, y_{a2},\dots, y_{an}$. Similarly, the 
vector $y_a$ contains $n$ data points: 
$y_{b1}, y_{b2},\dots, y_{bn}$

Assume also that the data are \textbf{paired}: the first pair of data points $y_{a1}$ and  $y_{b1}$ comes from the same subject, the second set from another subject, etc.

If we want to know the average difference in reading times between the two conditions, our null hypothesis will be that the difference in means is 0:
...
Isolate the relevant columns, and aggregate the data such that we have one set of paired data for each subject:
...
The empirical density plots shown above don't seem to show a normal distribution, as assumed in the equation we wrote at the beginning of this chapter:
...
It is pretty common in psycholinguistics to ignore this violation of the modeling assumption! For now, we will also ignore it; later on, we will return to this point. 
...
For comparison, the published result in \cite{grodner} is:  t1(1, 41) = 11.9, MSE = 8000, p $<$ .001; t2(1, 15) = 14.3, MSE = 2400, p $<$ .01.

Now we will fit a \textbf{linear model} to these data.

<<>>=
aggregated_data<-data.frame(subj=rep(1:42,2),cond=factor(rep(c("or","sr")),each=42),rt=c(OR,SR))
m0<-lm(rt~cond,aggregated_data)
summary(m0)
@

This linear model fits the same model as the one-sample t-test above. It's important to understand what lies behind this output. We won't go through everything in the output, but only focus on some aspects that are immediately of relevance to us. First, we have the coefficients, which define the \textbf{intercept} and \textbf{slope} of the fitted line respectively:
...
The mean for SRs is xxx, and the mean for ORs is xxx+xxx=xxx:

The fitted linear model implies two generative processes for the two conditions:

ORs:

$y =  xxx + \varepsilon$ where $\varepsilon \sim Normal(0,xxx)$

SRs:

$y =  xxx + xxx +  \varepsilon$ where $\varepsilon \sim Normal(0,xxx)$

This is encoded in the data frame by coding rows containing object relatives as 0's and rows containing subject relatives as 1's.
...
and plot them by comparing them to a standard normal distribution: 
...
It's worth understanding a little bit about what these parts of the linear model are.
...
\textbf{Vector of parameters to be estimated}:
...
What do you think the coefficients mean? Stop and work this out before reading further.

We can also examine the effects of noise and degree together:

<<>>=
## evaluating effect of noise AND degree together:
m2<-lm(rt~noise+deg,MD497.df)

results.m2<-extractfit(m2,
                        coln=c("","coef.",
                               "SE","t-value"),
                        indices=1:4,
...
<<results='asis',echo=F>>=
myxtable(results.m3,
         cap="The effect of beauty level on teaching evaluation score.",
         lab="tab:m3")
@
